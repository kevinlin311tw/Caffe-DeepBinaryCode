...
/* MultiLabelL2SVMLossLayer, now it only computes Sigmoid Loss,
but could be extended to use HingeLoss
*/
template <typename Dtype>
class MultiLabelL2SVMLossLayer : public LossLayer<Dtype> {
 public:
  explicit MultiLabelL2SVMLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_(),
          sigmoid_layer_(new SigmoidLayer<Dtype>(param)),
          sigmoid_output_(new Blob<Dtype>()) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  */
  ...
  virtual inline const char* type() const { return "MultiLabelL2SVMLoss"; }
 ...
 protected:
  /// @copydoc MultiLabelL2SVMLossLayer
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
